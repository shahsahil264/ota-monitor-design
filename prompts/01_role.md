...
**Give the human a choice of email address or Slack @-mention — both are valid, ask for either.** Earlier guidance said never to ask for an email directly, based on an evaluator block observed on OCPBUGS-122096 (`[OTA-Monitor]` PII-solicitation rule). That block was traced to an overly-broad platform-level privacy rule, which the platform team fixed at the rule level (confirmed ~Sep 10). **Verified directly with a live test on OCPBUGS-123612 (Sep 15): asking for "the assignee's email address" now goes through cleanly, no evaluator block.** A Slack @-mention was also tested on the same bug and resolved correctly to the right person's email. Both paths work today — this is now a matter of convenience, not compliance.

1. Before creating the Spike, ask the human: "Who should be assigned to this Spike? Please provide their email address or @-mention them in Slack (this is the person expected to answer the impact statement questions — typically the same person assigned to the OCPBUGS bug)."
2. If the human @-mentions someone, Slack resolves it to a `@UXXXXXX` user ID client-side before the message reaches the bot. Call `resolve_slack_user` with that user ID to get their email. If the human provides an email address directly, use it as-is — no preference between the two, whichever the human finds more convenient.
3. Create the Spike issue (per Spike Creation Details below).
4. Call `assign_jira_issue` on the newly created Spike with the resolved (or provided) email.

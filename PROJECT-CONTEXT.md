...
18. **Spike assignee — design assumption was wrong, now FIXED in `01_role.md` (Sep 1).** Our spec originally said "Assignee: Same as the OCPBUGS bug's assignee," assuming this was automatic. On a real button click (OCPBUGS-114720), the bot reported it genuinely cannot see assignee at all — confirmed architectural, not a bug: both the Snowflake/Dataverse content path (PII-stripped) and the live Jira API metadata path deliberately exclude assignee/reporter/author, with no privileged bypass, by design, for privacy. Confirmed this is the same for every persona on the platform — not something specific to OTA Monitor. **Fix (code-free, prompt-only):** added a new "Gate 1: Collecting the Assignee" step — when the human clicks [Create Spike], the bot now asks for the assignee's email as part of that same interaction, creates the Spike, then calls `assign_jira_issue` (already exists, no ownership check, accepts any email) with the provided address, before continuing with linking/labeling/commenting. One interaction, not a manual follow-up to remember. Updated the Spike Creation Details "Assignee" line to state the real constraint instead of the false assumption. Committed to design repo (`a8fe089`).

## 15. Open Items / Not Yet Resolved (updated Sep 8 — label bug RESOLVED)

- **`update_only_self_created` blocking label updates — RESOLVED (Sep 8), confirmed fixed via a platform PR that went beyond what we recommended.** Full history below is kept for context, but the short version: **PR #522 ("New personas / minor fixes"), merged Sep 1 (`7cdd430`) and confirmed deployed, completely redesigned the ownership check.** `jira_ops.py`'s `priv_jira_update_issue` no longer has an `only_self_created: bool` parameter at all — verified directly in the current source. It's replaced with `allow_full_edit: list[str] | None = None`, a **per-project allowlist**: when `None` (default), the old bot-created-only guard still applies; when it lists project keys, those specific projects bypass the guard entirely, while every other project across every other persona keeps the original protection. OTA Monitor's config (`config/groups/art-release/personas/ota_monitor.yaml`) now sets `allow_full_edit: [OCPBUGS, OTA]` — verified directly, with an inline comment warning "a wildcard would lift the bot-created-only guard everywhere." This is architecturally tighter than our recommended option (b) (which would have been an all-or-nothing bypass for OTA Monitor); a scoped allowlist is a better security boundary. **How we caught the tracker was stale:** OCPBUGS-115317, created under a personal (non-bot) Jira account, had two label-update operations succeed cleanly (Rule 3 auto-transition, then Gate 2 accept) — direct evidence contradicting an "still broken" status chai-bot had given us, which prompted the re-check that surfaced PR #522. **Lesson: verify a "still open" status against live behavior before accepting it, even from a source that's been reliable — trackers/status reports can go stale as fixes land through parallel unrelated work.**

  <details>
  <summary>Original root-cause investigation (Aug 31, kept for history)</summary>

  ROOT CAUSE CORRECTED AND CODE-VERIFIED (Aug 31); it was, at the time, a dead-config bug, not a misapplied override. The original platform theory was that the Aug 13 onboarding override to `false` "was never applied, was applied incorrectly, or was reverted." We disproved this directly: `git log -p -L13486,13487:config/workspaces.yaml` against `openshift-eng/ship-help-bot` shows exactly one commit ever touched that config line — `e578878d` ("Add OTA Monitor persona with 3 scheduled tasks and Jira workflows", Justin Pierce, **Aug 12, 2026 22:51:02 -0400**, the persona's original creation commit), whose own message says "label updates on OCPBUGS (approval-gated, only_self_created disabled)." `update_only_self_created: false` has been correctly set, continuously, since day one — never reverted, never re-applied, nothing to "fix" at the config level.

  The real bug (confirmed by reading `ship_help_bot/tools/jira/__init__.py` and `jira_ops.py` directly, not just from chai-bot's description): there are **two code paths**, and only the unused one reads the config.
  - `Toolset._build_jira_priv_tools()` (`ship_help_bot/tools/jira/__init__.py:217-242`) tries `build_privileged_tools(operation_keys=["jira_create_issue", "jira_update_issue"])` first (`shared/priv_ops.py:670` — a generic, persona-agnostic registry builder shared with `scm_api`, `remote_workspace`, and `art_build_tools`, so it has no way to thread per-persona config through). This registers `priv_jira_update_issue` (`jira_ops.py:84-115`), which hardcodes `only_self_created: bool = True` as a plain default parameter — not in `param_display`, not read from `self.config` anywhere, no path to the persona's `JiraConfig` at all. This is the path that actually runs.
  - `Toolset._build_update_fn()` (`__init__.py:67-93`) correctly reads `self.config.update_only_self_created` via closure — but it's only reachable from the `except Exception` fallback branch of `_build_jira_priv_tools()` (line 226), i.e. only if `build_privileged_tools()` itself throws. In normal operation it never executes.

  So the persona config has been silently disconnected from runtime behavior since the persona was created — not a regression, not a deploy-lag issue, a bug present from day one that a "re-apply the override" fix would have been a no-op against.

- **Fix options (given the confirmed code path), pick one:**
  - (a) Thread the persona's config value into `build_privileged_tools()`'s construction of `priv_jira_update_issue` at registration time — mirrors what `_build_update_fn()`'s closure already does, but requires the shared, persona-agnostic `build_privileged_tools()` framework to accept per-operation config, which also touches `scm_api`/`remote_workspace`/`art_build_tools` call sites. Broadest blast radius.
  - (b) Have `priv_jira_update_issue` read the config at call time via `tool_context` (the function already accepts `tool_context=None` and does nothing with it) — no change to the shared `build_privileged_tools()` framework, scoped entirely to the Jira toolset. **This is the one we'd recommend**: smallest surface area, consistent with how the codebase already threads per-persona config into tool calls elsewhere (`ConfigurableFunctionTool` + `tool_config` injection, used two lines above in this same file for `get_jira_issue`'s Snowflake config).
  - (c) Expose `add_issue_labels` (`client.py:744` — atomic label-add, no ownership check, currently only wired to the `jira_solve` RWS plugin profile) as a general-purpose tool. Architecturally cleaner long-term (label-add is lower-risk than a full issue update and shouldn't need the ownership gate at all) but doesn't fix the underlying disconnect for `priv_jira_update_issue` itself — a parallel improvement, not a substitute for (a)/(b).

**Status as of Sep 1 (superseded — see resolution above):** corrected root cause + verified code citations sent to chai-bot; recommended option (b). **Why chai-bot couldn't open this PR itself:** its prompt-file PRs (#417, #519) are markdown syncs scoped to one persona's own instruction directory — a fundamentally lower risk tier. This fix touched `jira_ops.py`, the shared authorization gate for privileged Jira writes across ALL personas — a deliberate scope boundary: bot-authored PRs against shared security infrastructure come from the platform team with human design review of cross-persona impact. **Resolved Sep 8 by PR #522, actually merged Sep 1 in parallel — the platform team was already working this before our escalation reached them, and shipped a better design (per-project allowlist) than the option we recommended.**

  </details>

- **PR #519 CI question — unresolved, genuinely unknown whether current `main` passes its unit tests.** File-level theory eliminated: all 5 OTA Monitor instruction files on `main` (`01_role.md`, `ota-component-mapping.yaml`, `ota_monitor_brief.md`, `ota_monitor_enriched.md`, `ota_monitor_weekly_handover.md` — the latter 3 live under `shared/instructions/scheduled/`, not the `ota_monitor/` directory) were independently pulled and confirmed clean of bare curly-brace placeholders. So the failing `ci/prow/unit` check is either (a) an unrelated failure in the ~3,762-test suite that happens to surface via a test that constructs all personas, or (b) a pre-existing/flaky failure on `main` unrelated to PR #519 at all. Neither we nor chai-bot can reach the Prow log (both hit a 403 behind QE-private-deck SSO auth) or trigger a post-merge CI run. **Confirmed fact: zero CI check runs exist against the merge commit `4a9149c` as of writing — `main` has not been validated post-merge at all.** Needs someone with actual Prow/cluster auth (platform team, or Sahil via VPN/SSO) to either pull the real error text from the failed run on `a930c646`, or confirm/trigger test health on current `main`. Logged in the tracker; treat as open until the actual error text is in hand — no further speculation until then.
**Deployment visibility (related finding):** there is no user-facing deployment notification system — deployments are batched and triggered manually by the platform team with no public announcement or pre-deployment warning. Practical guidance given: for scheduled runs, a deployment landing mid-window kills that cycle but the next cron tick should succeed (this was the first occurrence since go-live, described as rare). For on-demand runs, if a manual trigger seems to hang or produce no output, a deployment may be in progress — wait a few minutes and retry rather than assuming a real failure. A "deploying now" notification channel was suggested as a low-priority feature request.

- **Spike assignee via Slack @-mention — RESOLVED (Sep 10-11), via a new tool rather than the one we originally asked for.** Following up on Finding 18 (Gate 1 assignee collection): in production use, a human naturally responded with a person's name rather than an email when asked for the assignee. We asked for `lookup_slack_user_ldap` (in `forum_ops`) to be exposed to OTA Monitor; the platform team's engineering backlog kept that extraction pending, but instead shipped something better: a brand-new standalone tool, `resolve_slack_user`, in `ship_help_bot/tools/_auto/identity_resolution/` with `auto_register = "always"` — meaning it's automatically available to **every** persona, including OTA Monitor, with **zero config changes needed**. Confirmed deployed Sep 11. It takes a Slack user ID (which Slack itself resolves client-side from an `@-mention` before the bot ever sees the message) and returns kerberos ID, email, full name, and cost center via LDAP — functionally exactly what we needed.

  **This was picked up automatically**: Chai Bot's own post-processing self-improvement mechanism opened PR #635 against `01_role.md` independently, without us asking — it noticed a real, separate bug (the Gate 1 instructions we wrote in PR #538 literally told the bot to ask for "the assignee's email address," which **directly violates the platform's base system instruction never to solicit PII** — a real evaluator-blocked failure on OCPBUGS-122096) and fixed both the PII violation and the resolution mechanism in one PR, using `resolve_slack_user` for the Slack-handle path.

  **One gap found reviewing PR #635:** the PR's instructions also route name/Kerberos-ID input through OrgData (`search_employees`, `get_employee_by_uid` in `experts/orgdata/tools.py`) — these functions genuinely exist and would work (contradicting an earlier, incomplete answer from chai-bot that OrgData "isn't designed" for person lookup), but they're gated behind an `individual_lookups_enabled` config flag for defense-in-depth against accidental PII leakage, and OTA Monitor's config does not set this flag. Flagged directly on the PR, along with a real wording bug (step 2 described `resolve_slack_user`'s input/output direction backwards) — chai-bot pushed a corrected version reflecting our narrowed, @-mention-first design, matching what we'd already committed to our own design repo (`1f6264e`).

  **PR #635 closed intentionally, not accidentally — CORRECTED (Sep 15), initial "regression" alarm was wrong.** We initially flagged this as an active, urgent, still-live bug after seeing the unchanged Gate 1 wording on `main`. Chai-bot pushed back with a more specific claim: the Chai team traced the original OCPBUGS-122096 evaluator block to an overly-broad **platform-level** privacy rule (it banned all identifier solicitation, including legitimate work-related ones), fixed the rule itself around Sep 10, and closed PR #635 as "working around the wrong problem" — i.e., the prompt was never the actual bug, the platform rule was. Rather than accept or reject that claim on argument alone, we ran a real test: created OCPBUGS-123612, triggered Gate 1 Spike creation, and watched what happened. **Result: the bot asked "Please provide the assignee's email address" directly, and it went through cleanly — no evaluator block, no reword.** This confirms chai-bot's account was correct and our alarm was not. Current status: **not a bug, not urgent.** The @-mention-first design from our design repo (`1f6264e`) is still worth deploying eventually as a UX improvement (more natural to @-mention a colleague than look up their email), but there's no correctness or compliance issue forcing it, and it can be synced whenever convenient rather than as a fire drill. **Lesson: verifying "is this actually still broken" by testing beats verifying by re-reading source a second time — the source told us the wording was unchanged, which was true, but didn't tell us whether the *enforcement* behind it had changed. When a claim is about runtime behavior (does X still get blocked), a real test beats re-inspecting static config every time.**

  **Follow-up test (Sep 15, same OCPBUGS-123612 Spike): the Slack @-mention path ALSO works today, even though it's not yet in the deployed prompt.** Replied to the bot's "please provide the assignee's email address" prompt with an `@`-mention instead of typing an email — the bot correctly resolved it to the real person's email and completed Gate 1 (Spike `OCPNODE-4751` created, assigned to the mentioned person, linked, labeled, memory-recorded). So both input paths work today: literal email (deployed, always worked once the platform rule was fixed) and Slack @-mention (works even though the currently-deployed `01_role.md` never explicitly documents or instructs it — the model handled it sensibly on its own, likely reaching for `resolve_slack_user` unprompted). **Design repo updated (commit `98a948f`)** to explicitly document both paths as equally valid: Gate 1 now asks "please provide their email address or @-mention them in Slack," with no preference between the two — this closes the gap between what's documented and what's actually already working, and makes the @-mention path something the bot will reliably do by instruction rather than by luck. Requested chai-bot sync this into ship-help-bot via a fresh PR from design repo HEAD.

  **PR #703 regression caught before merge (Sep 15):** the resulting sync PR (#703) would have REMOVED PR #662's Recent-Actions Memory section entirely — because our design repo's `01_role.md` had never been updated with #662's content in the first place (that fix landed via chai-bot's direct commit to ship-help-bot, not through our normal design-repo → PR-sync workflow, so it never flowed back). Chai-bot's own reviewer caught this before merge and flagged it rather than merging blind. Fixed by merging PR #662's exact deployed content (Recent-Actions Memory section + updated ~3h Snowflake lag text) into the design repo alongside our own Gate 1 change (commit `0c78a57`) — verified line-by-line against the live deployed file, not just assumed correct. **This is the second time a platform-originated fix (PR #522's `allow_full_edit`, #635's evaluator-rule change, #662's Memory collection) has landed without flowing back into our design repo — reinforces that the design repo needs to be treated as potentially stale after ANY platform-side change, not just ones we didn't request. Before requesting any future sync PR, diff our design repo's target file against the live deployed version first, not just add our own change on top of what might be an outdated base.**

- **Snowflake sync-lag re-alerting fix — RESOLVED and deployed (PR #662, merged Sep 15).** Verified directly: `merged: true`, merge commit `34db0ad4`, CI green (lint/unit/images all `success` — no repeat of earlier CI-red merge incidents). Adds the `ota_monitor_recent_actions` memory collection exactly as designed (persona scope, 1d TTL, `bug_key`/`action_type`/`timestamp` schema) plus the write-before-comment ordering and additive idempotency checks across Rules 1/3/4/4b/6. **Not yet tested end-to-end** — next step is to actually verify the fix on a real or test bug (skip something, then trigger a rescan within the old ~3h lag window, confirm no re-alert) once deployment is confirmed live, not just merged.
...
- **Snowflake sync lag causes real re-alerting on Skip/Feedback actions — ACTIVE ISSUE (Sep 11), fix designed, blocked on a config PR.** A teammate clicked [Skip] on OCPBUGS-121851 at 08:25 UTC (Jira comment written immediately via the live REST API); the 10:00 UTC enriched scan — only 1h35m later — re-proposed the exact same bug as a new alert, and a second Skip click still didn't stop it that same day. Chai-bot's own first-pass explanation ("the daily status report doesn't currently check for Feedback comments") was **verified false** — we pulled the deployed `ota_monitor_enriched.md` directly and Rule 6 already has this exact check, verbatim. The real cause, confirmed against the timeline: Rule 6 reads prior comments via the Snowflake `CLOUDRHAI_MARTS` sync (the same ~3-hour empirical lag already documented in `01_role.md`'s Known Edge Cases), and the Skip comment genuinely hadn't synced yet when the scan ran. This is meaningfully worse than the previously-documented Snowflake-lag case (duplicate `[OTA-Monitor] Spike offered` comments during rapid on-demand re-scans, low-impact) — this one re-proposes something a human explicitly and deliberately dismissed, which is exactly the failure mode the idempotency system exists to prevent. Also a good example of the bot confabulating a wrong root cause when asked conversationally instead of checking its own deployed instructions or documented edge cases first — worth remembering as a pattern.

  **Agreed fix (design confirmed with chai-bot, not yet built):** use the platform's **Memories** capability (Firestore-backed, persona-managed structured storage, distinct from both Snowflake and the live Jira API — no sync lag) as an additive overlay, not a replacement for Jira. New memory collection `ota_monitor_recent_actions` (persona-scoped — visible to both scheduled scans and interactive sessions), schema `{bug_key, action_type, timestamp}`, TTL 1 day (comfortably beyond the ~3h empirical lag). **Every** gate action (not just Skip — Create Spike, Escalate, Accept, Revise, Not a Blocker, Add FixedIn, Extend Risk all share the same latent race condition, since they all rely on a Snowflake-read comment as their idempotency signal) writes a record to this collection **before** writing the Jira comment — so a partial failure between the two steps still leaves the memory-based protection in place, and a human can safely retry. Every detection rule's idempotency check adds an OR condition: existing Snowflake/label/linked-issue checks (unchanged, remain the durable long-term source of truth) **OR** a matching recent-actions memory entry (closes the short-term race). If the memory entry ages out, Jira comments remain the permanent fallback record — this only ever adds protection, never removes the existing safety net.

  **Blocked on:** a Chai-team config PR to add the `ota_monitor_recent_actions` memory collection to the persona config (same tier of change as adding a tool or datastore — not something we can do via a prompt-only PR). Once that lands, our own follow-up is a prompt PR to `01_role.md` (gate action handlers write to the collection) and `monitor-enriched.md` (detection rules read from it) — normal PR-sync process once the collection exists.

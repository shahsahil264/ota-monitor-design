...
**Agreed path forward (Aug 31):** ship option (a) — the `workspaces.yaml` config flag flip — immediately to unblock production, with option (b) as a follow-up architectural improvement if the team wants it later. No owner or ETA visible from outside the platform team; the bot's own recommendation was to wait a couple of working days for the tracker (which now has full root cause, code references, exact fix, and 3 confirmed occurrences) to get picked up, and escalate via `@help-chai` directly if it doesn't. **Verification plan once the fix ships:** test on OCPBUGS-114342 specifically (the real bug that still needs a label added) to confirm end-to-end before trusting it on the next live detection.

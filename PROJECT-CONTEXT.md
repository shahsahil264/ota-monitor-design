...
## 15. Open Items / Not Yet Resolved

- Scheduler occasionally misses a run entirely (no post at all) — reported, root cause not yet confirmed (scheduler-level, ruled out prompt regression via on-demand test)
- Pipeline check sometimes reports "data unavailable" even when the channel is confirmed indexed and searchable via direct interactive query — reported, 3 hypotheses given (query formulation, scheduled-vs-interactive context differences, indexing lag), unresolved
- `update_only_self_created` blocking label updates on non-bot-created bugs — reported, root cause theory given (comments vs. labels go through different code paths with different ownership guards), unresolved
- Duplicate idempotency-marker comments from rapid on-demand re-scans — reported, likely Snowflake sync lag related, unresolved
- Self-approve policy inconsistency (Jira vs. GitHub) — reported as a design question to the platform team, no resolution yet

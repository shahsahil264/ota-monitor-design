...
- **Spike assignee via Slack @-mention — RESOLVED (Sep 10-11), via a new tool rather than the one we originally asked for.** Following up on Finding 18 (Gate 1 assignee collection): in production use, a human naturally responded with a person's name rather than an email when asked for the assignee. We asked for `lookup_slack_user_ldap` (in `forum_ops`) to be exposed to OTA Monitor; the platform team's engineering backlog kept that extraction pending, but instead shipped something better: a brand-new standalone tool, `resolve_slack_user`, in `ship_help_bot/tools/_auto/identity_resolution/` with `auto_register = "always"` — meaning it's automatically available to **every** persona, including OTA Monitor, with **zero config changes needed**. Confirmed deployed Sep 11. It takes a Slack user ID (which Slack itself resolves client-side from an `@-mention` before the bot ever sees the message) and returns kerberos ID, email, full name, and cost center via LDAP — functionally exactly what we needed.
  ...
  **REGRESSION DISCOVERED (Sep 15): PR #635 was closed WITHOUT merging, and the underlying PII bug is still live in production.** Verified directly — GitHub API shows `merged: false`, no `merged_at`. Checked the currently-deployed `01_role.md` on `main` and Gate 1 still has the ORIGINAL broken wording: *"Please provide the assignee's email address."* PR #662 (Recent-Actions Memory, merged Sep 15 — see below) touched a different section of the same file and never fixed this. So despite all the review back-and-forth that appeared to resolve this, **the PII-solicitation bug that originally prompted PR #635 has been silently live and unfixed for 5 days**, with every Spike creation presumably still hitting the same evaluator-block failure mode as the original OCPBUGS-122096 incident. Requested a fresh sync PR from design repo HEAD (not resurrecting #635) and asked chai-bot to explain why the PR was closed instead of merged. **Lesson reinforced: a PR being reviewed, discussed, and seemingly agreed-upon is not the same as it being merged and deployed — always verify final state, don't assume a resolved-sounding conversation means the fix shipped.**

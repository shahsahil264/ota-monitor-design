...
- **Spike assignee via Slack @-mention — pending Chai team packaging decision (Sep 8).** Following up on Finding 18 (Gate 1 assignee collection): in production use, a human naturally responded with a person's name rather than an email when asked for the assignee. Chai-bot identified a real fix: Slack @-mentions resolve to a `@UXXXXXX` user ID client-side before the bot ever sees the message, and `lookup_slack_user_ldap` (an existing platform tool) can resolve that user ID to an email via Red Hat LDAP — no free-text name search needed at all. **Verified directly against source and deployed config, not just chai-bot's description:** `lookup_slack_user_ldap` is real, defined in `ship_help_bot/tools/forum_ops/tools.py` — but OTA Monitor's persona config (`tool_profile: ship_standard` + `tools/jira` only) does not include `forum_ops`, and `ship_standard` itself doesn't bundle it either. Confirmed `forum_ops` bundles far more than the LDAP lookup: `classify_forum_ticket`, `update_forum_ticket`, `query_forum_tickets`, `get_onboarding_support_summary`, `search_public_personas`, `dm_entitled_personas`, `read_bot_source`, `grep_bot_source`, `update_canvas`, `check_pr_deployment_status` — all support-channel/dev-tooling functions built for #chai-users, irrelevant to OTA Monitor's mission. Chai team is now assessing the right packaging approach (candidates: register `lookup_slack_user_ldap` individually outside the package, move it into a shared identity-utilities module or `orgdata`, or just accept the whole package as-is) — logged as a config request on their end. **Our next step, blocked on them:** once the tool is available in whatever form, update `01_role.md`'s Gate 1 assignee step to accept a Slack @-mention as an alternative to a literal email, resolving via `lookup_slack_user_ldap` with the plain-email ask as fallback.
